...
| 구성 요소 | 상태 |
|---|---|
| 해석 엔진 + 환각 차단 + 멀티턴 | ✅ 완성 (테스트 30개 통과) |
| 롯데리아 메뉴 50종 + 표현 사전 + 테스트셋 222건 | ✅ 완성 (규칙 기준선 98.6%) |
| STT(CLOVA)·TTS(Google) 프로바이더 + /order | ✅ 구현 완료 — **실키 연동 확인 필요** (`scripts/smoke_voice.py`) |
| 키오스크 UI (음성·터치 병행, 세트/알레르기 표시) | ✅ 구현 완료 |
| 접근성 | ✅ 명도 대비 전 조합 4.5:1 이상, 터치 표적 48dp 이상 |
| 라즈베리파이 통합 | ⬜ 예정 |
| 현장 실증 (경로당·복지관) | ⬜ 7/6~7/10 예정 — 성능 수치는 실증 후 기재 |
